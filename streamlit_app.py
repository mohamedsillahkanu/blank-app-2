...
# Custom CSS for Blue Highlight Styling
st.markdown("""
    <style>
        [data-testid="stSidebar"] {
            background-color: #0E1117 !important;
...
# Sidebar with two collapsible sections
with st.sidebar:
    st.title("🔹 Main Menu")
...
selected = selected_data if selected_data else selected_settings

# Mapping selected submenu to corresponding Python files
file_mapping = {
    "Data Management": "interactive.py",
    "Data Analysis": "data_analysis.py",
    "General Settings": "settings.py",
    "User Management": "user_management.py"
}

file_path = file_mapping.get(selected, None)
...
if file_path:
    ...
    try:
        ...
        st.subheader(f"📄 {file_path} Content:")
        edited_code = st.text_area(f"Edit {file_path}:", code_content, height=300)

        # Save changes if edited
        if st.button("💾 Save Changes"):
            with open(file_path, "w") as file:
                file.write(edited_code)
            st.success(f"Changes saved successfully to {file_path}!")

        # Run the Python file and display output
        if st.button(f"▶️ Run {file_path}"):
            result = subprocess.run(["python", file_path], capture_output=True, text=True)
            st.subheader("🖥️ Output:")
            st.code(result.stdout if result.stdout else "No output", language="plaintext")
            if result.stderr:
                st.error(result.stderr)

    except FileNotFoundError:
        st.warning(f"File `{file_path}` not found. Please check if it exists in the directory.")

else:
    st.error("No file mapped to this menu option.")
