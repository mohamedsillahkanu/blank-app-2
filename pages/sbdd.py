...
if uploaded_file:
    ...
    with col1:
        ...
        if district_df['Total_Enrollment'].sum() > 0:
            ...
            gender_disparities = []
            for d in summaries['district']:
                if d['boys'] > 0:
                    gender_gap = abs(d['gender_ratio'] - 100)  # Deviation from perfect parity (100%)
                    gender_disparities.append((d['district'], gender_gap, d['gender_ratio']))
            gender_disparities.sort(key=lambda x: x[1], reverse=True)
    ...
    if map_images:
        ...
        with st.expander("📁 View Saved Map Files"):
            ...
            if 'enrollment_itn' in map_images:
                ...
                chart_run = chart_para.add_run()
                map_images['enrollment_itn'].seek(0)
                chart_run.add_picture(map_images['enrollment_itn'], width=Inches(6))
            ...
            if 'gender_district' in map_images:
                ...
                chart_run = chart_para.add_run()
                map_images['gender_district'].seek(0)
                chart_run.add_picture(map_images['gender_district'], width=Inches(6))
