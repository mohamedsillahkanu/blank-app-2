...
try:
    ...
    with col6:
        # Save high-resolution PNG with title outside plot
        output_path_png = "health_facility_map.png"
        
        # Create a new figure for saving
        save_fig = plt.figure(figsize=(20, 28))  # Increased height for title space
        
        # Add title at the top with padding
        plt.suptitle(map_title, fontsize=24, y=0.98)
        
        # Copy the map plot to the new figure
        map_ax = save_fig.add_subplot(111)
        map_ax.axis('off')
        
        # Copy content from original figure
        original_ax = fig.get_axes()
        for idx, chiefdom in enumerate(chiefdoms[:20]):
            if idx >= n_rows * n_cols:
                break
                
            # Create subplot in the same grid
            ax = plt.subplot(n_rows, n_cols, idx + 1)
            
            # Copy the content from original plot
            chiefdom_shapefile = district_shapefile[district_shapefile['FIRST_CHIE'] == chiefdom]
            chiefdom_shapefile.plot(ax=ax, color=background_color, edgecolor='black', linewidth=0.5)
        ...
        plt.tight_layout()
        plt.subplots_adjust(top=0.94)  # Leave space for title
        
        # Save the figure
        save_fig.savefig(output_path_png, 
                        dpi=300, 
                        bbox_inches='tight', 
                        pad_inches=0.5,
                        facecolor='white')
        plt.close(save_fig)  # Close the temporary figure
