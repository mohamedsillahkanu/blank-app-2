...
try:
    ...
    # Get all available columns from facility data
    available_columns = [col for col in facility_data.columns if col not in ['w_long', 'w_lat', 'geometry']]
    
    # Let user select which columns to show in hover
    st.subheader("Hover Information")
    default_columns = ['facility', 'FIRST_CHIE']  # Always show these
    additional_columns = st.multiselect(
        "Select additional information to show when hovering over facilities",
        available_columns,
        default=[]
    )
